...
void create_log(clock_t time) {
    ...
    for (int i = 0; i < n; i++) {
        fprintf(f, "Task No %d\n", i + 1);
        fprintf(f, "Waiting time in the first line: %ld\n", timer.get_waiting_times()[i][0]);

        fprintf(f, "Start time of processing in the first container (from the start of the program): %ld\n", timer.get_time_from_start()[i][0]);
        fprintf(f, "Processing time in the first pipeline: %ld\n", timer.get_working_times()[i][0]);

        fprintf(f, "Secondary placement time (from the start of the program): %ld\n", timer.get_time_from_start()[i][1]);
        fprintf(f, "Waiting time in the second queue: %ld\n", timer.get_waiting_times()[i][1]);

        fprintf(f, "Start time of processing in the second container (from the start of the program): %ld\n", timer.get_time_from_start()[i][2]);
        fprintf(f, "Processing time in the second pipeline: %ld\n", timer.get_working_times()[i][1]);

        fprintf(f, "Time of placing in the third stage (from the start of the program): %ld\n", timer.get_time_from_start()[i][3]);
        fprintf(f, "Waiting time in the third queue: %ld\n", timer.get_waiting_times()[i][2]);

        fprintf(f, "Start time of processing in the third container (from the start of the program): %ld\n", timer.get_time_from_start()[i][4]);
        fprintf(f, "rocessing time in the third pipeline: %ld\n", timer.get_working_times()[i][2]);

        fprintf(f, "Processing end time (from the start of the program): %ld\n", timer.get_time_from_start()[i][5]);

        fprintf(f, "Minimum waiting time in the queue: %ld\n", timer.get_min_time()[i]);
        fprintf(f, "Maximum waiting time in the queue: %ld\n", timer.get_max_time()[i]);
        fprintf(f, "Average waiting time in queue: %ld\n", timer.get_avg_time()[i]);
        fprintf(f, "Task execution time: %ld\n", timer.get_proc_time()[i]);
        fprintf(f, "----------------------------------------------------------------------------\n");
    }

    fprintf(f, "System uptime: %ld\n", time);
    fclose(f);

    for (int i = 0; i < n; i++) {
        cout << "____________________________________________";
        cout << i << "\n";
        for (size_t j = 0; j < timer.get_time_from_start().size(); j++) {
